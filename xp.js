...
client.on('ready', () => {
   var arrayOfStatus = [
        `${client.users.cache.size} usuarios`,//change this with your status
     ...
     setInterval(() => {
        var arrayOfStatus = [
        `${client.users.cache.size} usuarios`,//change this with your status
        `.help Para ver mis comandos!`,//change this with your status
        `${client.guilds.cache.size} Servidores!!`,
        `Hector#0002`,
        `${db.fetch(`status`)}`
    ];
...
client.on("guildMemberAdd", async (member) => {
  ...
  let data = await canva.welcome(member, { link: "https://thumbs.dreamstime.com/b/cool-rectangular-pattern-illustration-wallpaper-background-cool-rectangular-pattern-illustration-wallpaper-background-204972378.jpg" }) 
...
client.on("guildMemberRemove", async (member) => {
  ...
  let wChan = db.fetch(`remove_${member.guild.id}`)
