...
function getStatData(location , $){
  ...
  execute: async(client, message, args) => {
    ...
    if(!args[0]){
        return message.channel.send("Por favot ingrese un STEAMID64 valido");
    }
    ...
    request(UR_L, function(err, resp, body){
        ...
        if(KD == -1){
            return message.channel.send("Inválido, asegúrese de que su perfil no sea privado y que haya ingresado un STEAMID64 válido.");
        }
