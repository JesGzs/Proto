...
module.exports = {
  ...
  execute: async(client, message, args, data) => {
    ...
    if (page > tpages) return message.channel.send(`Pagina no encontrada.`)
let logs = []
data.logs.map((x, y) => {
    ...
    .addField(`**Lista de comandos personalizados**`, logs.length == 0 ? "No hay comandos encontrados!" : logs.join("\n"))
 .setFooter(`Mostrando pagina ${page}/${tpages}`)
