			...
			setTimeout(() => {
				...
				collector.on("collect", (msg) => {
					...
					if(msg.content.toLowerCase().indexOf(word) >= 0 && wordList.map((word) => word.toLowerCase()).indexOf(msg.content.toLowerCase()) >= 0){
						...
						return msg.channel.send(`Palabra invalida \n By: ${msg.author.toString()}`);
					}
				});
				...
				collector.on("end", async (collected, reason) => {
					if(reason === "time"){
						...
						message.channel.send(`Esta palabra fue encontrada por <@${reason}>`);
						winners.push(reason);
					}
					if(i < nbGames-1) {
						i++;
