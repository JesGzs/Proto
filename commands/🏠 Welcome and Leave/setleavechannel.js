...
module.exports = {
  ...
  if (!args[0]) {
    let b = await db.fetch(`remove_${message.guild.id}`);
    let channelName = message.guild.channels.cache.get(b);
    if (message.guild.channels.cache.has(b)) {
      return message.channel.send(
        `**El canal de despedida configurado en este servidor es \`${channelName.name}\` Channel!**`
      );
  ...
  try {
    let a = await db.fetch(`remove_${message.guild.id}`);

    if (a === channel.id) {
      return message.channel.send(
        "**Este canal ya está configurado como canal de despedida**"
      );
    } else {
      client.guilds.cache
      ...
      db.set(`remove_${message.guild.id}`, channel.id);

      message.channel.send(
        `**El canal de despedida se ha establecido con éxito en \`${channel.name}\`**`
      );
    }
    return;
  } catch (e) {
